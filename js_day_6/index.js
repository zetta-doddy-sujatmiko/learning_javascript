class Book {
  ...
  function purchaseBook(book, discountPercentage, taxPercentage, purchaseAmount, creditDuration) {
    ...
    for(let i = 1; i <= creditDuration; i++) {
      ...
      const thisMonthPayment = Math.trunc(totalPrice / creditDuration)
      payments.push({
        dueDate: dueDate.getDate() + "/" + (dueDate.getMonth() + 1) + "/" + dueDate.getFullYear(),
        payAmount: i == creditDuration ? remainingPayment : thisMonthPayment
      })
  
      remainingPayment -= thisMonthPayment
    }
  ...
  purchaseBook(Books['Sapiens'], 50, 11, 20, 6)